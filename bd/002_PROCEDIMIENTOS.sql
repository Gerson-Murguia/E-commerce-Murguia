...
as
begin
	IF EXISTS (SELECT * FROM usuario WHERE correo = @Correo and contrasena = @Password)
	begin
		SELECT IdUsuario,Nombres,Apellidos,Correo,Contrasena,EsAdministrador FROM usuario WHERE correo = @Correo and contrasena = @Password
	end
end
...
begin
	...
	begin
		...
		(@Nombres,@Apellidos,@Correo,@Password,@EsAdministrador)
